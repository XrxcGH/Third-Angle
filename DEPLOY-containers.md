...
> **This is not the route in use.** The site runs on an Oracle Cloud Always
> Free machine with Cloudflare in front of it, and that is documented in
> [DEPLOY.md](DEPLOY.md). This route needs the $5/month Workers Paid plan.
> It is kept because it works, it is tested, and it is the escape hatch if the
> Oracle free tier is withdrawn — which has been reduced once already.
